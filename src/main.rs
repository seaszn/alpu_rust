use std::{fs, io::Error, str::FromStr, sync::RwLock};
...
use exchanges::{types::Protocol, UniswapV2MarketState};
use networks::Network;
use serde_json::Value;
use types::{market::Market, MarketState, PriceTable, Route, Token};

use crate::{exchanges::init_exchange_handlers, handlers::NetworkHandler, types::OrganizedList};
...
async fn main() {
    ...
    let route = Route::new(MARKETS.to_vec(), &*USDC);
    let reserve_table = &generate_test_reserve_table();
    let price_table = get_test_price_table().await;

    let route_result = route.calculate_result(reserve_table, &price_table);

    if let Some(res) = route_result {
        println!("{}", res.ref_profit_loss);
    }
    // ------
...
fn generate_test_reserve_table() -> OrganizedList<MarketState> {
    ...
    list.add_value(MarketState::UniswapV2((
        65926531549407848924u128.into(),
        120729641134u128.into(),
    )));
